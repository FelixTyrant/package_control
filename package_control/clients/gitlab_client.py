...
class GitLabClient(JSONApiClient):
    def make_tags_url(self, repo):
        ...
        match = re.match(r'https://gitlab.com/(?P<user>[^/]+)/(?P<repo>[^/]+)/?$', repo)
        if not match:
            return False

        return 'https://gitlab.com/%s/%s/-/tags' % (match.group('user'), match.group('repo'))
    ...
    def make_branch_url(self, repo, branch):
        ...
        return 'https://gitlab.com/%s/%s/-/tree/%s' % (match.group('user'), match.group('repo'), quote(branch))
    ...
    def download_info(self, url, tag_prefix=None):
        ...
        tags_match = re.match(r'https://gitlab.com/(?P<user>[^/]+)/(?P<repo>[^/]+)/-/tags/?$', url)
        ...
        output = []
        if tags_match:
            (user_id, user_repo_type) = self._extract_user_id(tags_match.group('user'))
            ...
            user, repo, commit = self._user_repo_ref(url)
            ...
            output.append({
                'url': url_pattern % (user, repo, commit, repo, commit),
                'commit': commit
            })
    ...
    def repo_info(self, url):
        ...
        user, repo, branch = self._user_repo_ref(url)
        if (not user) and (not repo):
            return (None, None)
        ...
        info = self.fetch_json(self._make_api_url('', repo_id))
        ...
        output['readme'] = 'https://gitlab.com/%s/%s/-/%s/%s' % (
            user,
            repo,
    ...
    def user_info(self, url):
        ...
        user_match = re.match(r'https://gitlab.com/(?P<user>[^/]+)/?$', url)
        if user_match is None:
            return None

        (user_id, user_repo_type) = self._extract_user_id(user_match.group('user'))

        repos_info = self.fetch_json('https://gitlab.com/api/v4/%s/%s/projects' % (user_repo_type, user_id))
        ...
        for info in repos_info:
            ...
            if repo_output['readme']:
                repo_output['readme'] = 'https://gitlab.com/%s/%s/-/raw/%s/%s' % (
    ...
    def _extract_repo_info(self, result):
        ...
        return {
            ...
            'readme': result['readme_url'] or None,
    ...
    def _user_repo_ref(self, url):
        ...
        branch_match = re.match(
            r'https://gitlab.com/(?P<user>[^/]+)/(?P<repo>[^/]+)/-/tree/(?P<branch>[^/]+)/?$',
            url
        )
        ...
        repo_match = re.match(
            r'https://gitlab.com/(?P<user>[^/]+)/(?P<repo>[^/]+)($|/.*$)',
            url
        )
